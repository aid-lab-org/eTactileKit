...
def create_rectangular_pad(point, width, height, layer, net_name, angle=0, drill_size=0):
    return {
        "x": point[0],
        "y": point[1],
        "shape": "rect",
...
def create_circular_pad(point, diameter_x, diameter_y, layer, net_name, angle=0, drill_size=0):
    return {
        "x": point[0],
        "y": point[1],
        "shape": "circle",
    ...
    for pad in json_result["pads"]:
        ...
        if pad["shape"] == "circle":
            ax.add_patch(
                Ellipse(
                    (pad["x"], pad["y"]),
                    width=pad["diameter_x"],
                    height = pad["diameter_y"],
                    fill=True,
                    color=color,
                )
            )
        else:#if the shape is rectangular
            ax.add_patch(
