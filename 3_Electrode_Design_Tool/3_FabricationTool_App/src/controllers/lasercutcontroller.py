...
import cv2
from vtk import vtkCellPicker,vtkContourTriangulator
import heapq
import math
...
class LaserCutController:
    def __init__(self, ui_page):
        self.ui_page = ui_page

        # Connect signals from the UI for the laser cut editor-buttons
        ...
        # Connect signals from the UI for the laser cut editor-line edits
        ...
        # Initialize to keep track of drawing contours
        ...
        self.electrode_radius = self.read_parameter_mm(self.ui_page.diameter_input)/2
        self.electrode_spacing = self.read_parameter_mm(self.ui_page.gap_input)

        # Initialize variables for connectors
        self.connector_region_drawing_mode = False
       
        self.pad_height = self.read_parameter_mm(self.ui_page.pad_height)
        self.pad_width = self.read_parameter_mm(self.ui_page.pad_width)
        self.pad_spacing = self.read_parameter_mm(self.ui_page.pad_spacing)
        self.num_pads = int(self.read_parameter_mm(self.ui_page.num_pads))

        self.connector_segments = []        #line segments drawn to create a connector footprint
        self.connector_pads_data = []       #group data for the pads of the connector [[[center.x, center.y, width, height, angle] for each pad] for each connector]
        self.connector_polygons_data = []   #group data for the polygons of the connector pads [[list of polygons] for each connector]
        ...
        self.track_width = self.read_parameter_mm(self.ui_page.trace_width) #default value is 0.5mm
        self.routing_clearance = self.read_parameter_mm(self.ui_page.routing_clearance) #default value is 0.5mm
        self.routing_paths = []
        ...
        # Initialize variables for generating pdf
        self.polygons = []  #stores all polygons to be drawn in the pdf
    ...
    def get_output_file_name(self):
        options = QFileDialog.Options()
        filename, _ = QFileDialog.getSaveFileName(self.ui_page, "Get File Name", "", "PDF Files (*.pdf);;All Files (*)", options=options)
        return filename
    ...
    def erase_nearby_elements(self, eraser_center, eraser_tolerance = 1):
        ...
        new_stored_points = []
        for point in self.populated_electrode_points:
            dist = np.linalg.norm(point - eraser_center[:2])
            if dist > eraser_tolerance: #no need to compare the z coordinate
        ...
        for pads_group_index in range(len(self.connector_polygons_data)):
            ...
            for pad in pads_group:
                try:
                    dist = pad.distance(Point(eraser_center[:2]))
                    if dist < eraser_tolerance:
    ...
    def prompt_for_dimensions(self, length):
        ...
        if ok:
            self.scale_factor = dimension / length #calculate the scale factor by dividing the real-world dimension by the image dimension
    ...
    def calibrate_image_dimensions(self):
        ...
        self.extracted_edge_contour_points = []#clear the extracted edge contour points because the scale is changed
        self.update_display_content()
    ...
    def disable_image_view(self):
        if self.base_image is None:
            self.ui_page.show_warning("Please load an image before hiding")
            return
        #reset all the modes if any are existing
        self.reset_modes()
    ...
    def proccess_drawn_region(self):
        if len(self.drawing_points) > 2:
            ...
            if self.connector_region_drawing_mode:
                ...
                self.connector_region_drawing_mode = False #Connectors are updated when diplay is updated at the end of the function
                #update the operation mode label
                self.current_mode = "Default"
            ...
            if self.electrode_region_drawing_mode:
                # Get the region as a shapely polygon for further processing
                drawn_polygon = Polygon(points[:, :2])

                electrode_points = self.generate_electrode_points(drawn_polygon, distance=self.electrode_spacing)
    ...
    def generate_connector_footprint(self, start_end_pad_width, start_end_pad_height, middle_pad_width, middle_pad_height, num_middle_pads, middle_pad_spacing, holding_start_spacing, y_offset):
        ...
        # holding_start_spacing - spacing between the holding pad and the starting edge of the middle pad
    ...
    def place_connector_pads_on_lines(self, base_lines):
        '''return two lists
        a list: a list of information [ [ [center.x, center.y, width, height, angle] for each pad] for each connector]
        polygon data: a [[list of polygons] for each connector]'''
        pad_info = []
        polygon_data = []
        # exterior_curve = base_line.exterior
        # total_length = exterior_curve.length
        ...
        for segment in base_lines:
            ...
            base_connector_pads_data = self.generate_connector_footprint(start_end_pad_width=0,
                                                                         start_end_pad_height=0,
    ...
    def reset_transform(self):
        if self.elements_available_to_adjust == False:
            self.ui_page.show_warning("Please select elements to adjust")
            return
        if self.ui_page.get_confirmation("Resetting Transform","Are you sure you want to reset the transformation?") == False:
            return
    ...
    def generate_conductive_traces(self):
        ...
        def get_unrouted_electrodes_connectors(electrode_centers, connector_centers):
            electrodes = []
            connectors = []
            if self.routing_paths:
        ...
        if len(self.extracted_edge_contour_points) > 0:
            ...
            for connector_group_data in self.connector_pads_data:
                #in the laser cutter  editor there is no ground pads but there are empty pads
            ...
            #get all the routable connector points
            ...
            unconnected_electrodes, unconnected_connectors = get_unrouted_electrodes_connectors(electrode_points, connector_points)
    ...
    def generate_output_files(self):
        def shapely_to_patch(polygon, **kwargs):
            """Convert a Shapely polygon to a Matplotlib patch."""
            if not polygon.is_empty:
                return patches.Polygon(list(polygon.exterior.coords), **kwargs)
            return None
        
        def plot_polygons_to_pdf(polygons, output_filename, scale=1.0):
            """
            Plot a list of Shapely polygons to a PDF with a 1:1 scale.

            :param polygons: List of Shapely Polygon objects.
            :param output_filename: The name of the output PDF file.
            :param scale: Scale factor for the plot. Default is 1.0 for 1:1 scale.
            """
            outline = LineString(self.extracted_edge_contour_points)
            outline_polygon = outline.buffer(0.1, cap_style=1, join_style=1)
            # Determine the bounds of the entire layout
            # all_bounds = [polygon.bounds for polygon in polygons]
            all_bounds = [polygon.bounds for polygon in polygons] + [outline_polygon.bounds]
            min_x = min(bounds[0] for bounds in all_bounds) - 1
            min_y = min(bounds[1] for bounds in all_bounds) - 1
            max_x = max(bounds[2] for bounds in all_bounds) + 1
            max_y = max(bounds[3] for bounds in all_bounds) + 1

            # ------------------------------------------------------Set up the Matplotlib figure for the main PDF plot------------------------------------------------
            fig, ax = plt.subplots()
            ax.set_aspect('equal', 'box')
            
            # Plot each polygon
            for polygon in polygons:
                patch = shapely_to_patch(polygon, edgecolor='black', facecolor='black', lw=0.5)
                if patch:
                    ax.add_patch(patch)
            ...
            # Set limits based on the bounds of all polygons, adjusted by the scale
            ...
            # Save the figure to a PDF with 1:1 scale
            with PdfPages(output_filename) as pdf:
                pdf.savefig(fig, bbox_inches='tight', pad_inches=0)

            plt.close(fig)  # Close the figure to free memory

            # -----------------------------------------------------------Set up the Matplotlib figure for the Mask PDF plot for electrodes------------------------------------------------
            fig, ax = plt.subplots()
            ax.set_aspect('equal', 'box')

            # add the electrodes to the polygons
            for electrode in self.populated_electrode_points:
                circle = Point(electrode).buffer(self.electrode_radius)
                patch = shapely_to_patch(circle, edgecolor='black', facecolor='black', lw=0.5)
                if patch:
                    ax.add_patch(patch)
            ...
            ax.axis('off')
            # Save the figure to a PDF with 1:1 scale
            with PdfPages(output_filename.replace('.pdf', '_mask.pdf')) as pdf:
                pdf.savefig(fig, bbox_inches='tight', pad_inches=0)          
            plt.close(fig)  # Close the figure to free memory

            # -----------------------------------------------------------  Get the contours and electrode coordinates   ------------------------------------------------
            # Get the contours and electrode coordinates
            import json
            # Convert each tuple into a dictionary with "x" and "y" keys
            outline_formatted = [{"x": float(x), "y": float(y)} for x, y in self.extracted_edge_contour_points]
            electrodes_formatted = [{"x": float(x), "y": float(y)} for x, y in self.populated_electrode_points]
            mapping_function = [i for i in range(len(self.populated_electrode_points))]
            ...
            # Save to a JSON file
            with open(file_name.replace('.pdf', '_coordinates.json'), 'w') as json_file:
                json.dump(data, json_file, indent=4)  # 'indent=4' makes the JSON more readable
        
        #---------------------------------------------------- Create Polygons from all the elements ----------------------------------------------------------------------
        # Now you have the outline | routing paths | connector pads | electrode pads
        # Adding the track polygons
        if self.routing_paths:
            for path in self.routing_paths: 
                line = LineString(path)
                # Buffer the LineString to create a polygon with the given track width
                track_polygon = line.buffer(self.track_width / 2.0, cap_style=1)  # cap_style=2 for square ends
                # Add the track polygon to the list
                self.polygons.append(track_polygon)
        
        # Adding the connector polygons
        for poly_group in self.connector_polygons_data:
            for poly in poly_group:
        ...
        if file_name:
            ...
            plot_polygons_to_pdf(self.polygons, file_name)

# ------------------------------------------- PATH SEARCH ON IMAGE ------------------------------------------------------------------
class TwoDRouter:
    ...
    def a_star(self, start, end, step, similarity_tolerance=1e-3):
        ...
        f_costs = {start: self.heuristic_euclidean(start, end)}  # cost based on the distance
        ...
        while start_points:
            ...
            for polygon in connector_polygons: #get only the connector polygon without the start point
                if not polygon.contains(Point(current_start)):
                    self.keep_out_polygons.append(polygon.buffer(self.track_width/2 + self.routing_clearance))
            #-----------------------------------------------------------------------------------------------------------------------------------------
            snapped_start = self.snap_point_to_grid(current_start, step)
            snapped_end = self.snap_point_to_grid(current_end, step)
