...
public class LoadData {
    ...
    public LoadData(PApplet parent) {
        this.parent = parent;
    }

    void load2D(String array_pattern) {
        try {
        json_array_data      = parent.loadJSONObject(array_pattern);
        ...
        System.err.println("Error loading JSON data: " + e);
        //Electrodes_number = 0;
    ...
    void load3D(String array_pattern, String object3D) {
        try {
        obj_shape = parent.loadShape(object3D);
        json_array_data = parent.loadJSONObject(array_pattern);
        //coordinates = json_array_Data.getJSONArray("outline");
        electrodes = json_array_data.getJSONArray("electrodes");
