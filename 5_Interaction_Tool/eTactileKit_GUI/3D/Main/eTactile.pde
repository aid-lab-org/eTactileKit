class ETactile{
  ...
  public void SETUP3D(String json_array, String COM_PORT, String Object3D){
    ...
    //dataLoader.load3DPattern(json_pattern);
  ...
  public void stimulate(String json_pattern){
    ...
    if (dataLoader.execution) {
      // If execution = true, communicate with microcontroller.
 ...
 //______________________________________________________________________//
  ...
  public void get_voltage_readings() {
    mySerialComm.sendByte((byte)PC_ESP32_MEASURE_REQUEST);
      ...
      for (int ch = 0; ch < number_of_electrodes; ch++) {
        Record[ch] = mySerialComm.readByteWithTimeout(0.020); //(byte)port.read();
      }
      for (int ch = 0; ch < number_of_electrodes; ch++) {
